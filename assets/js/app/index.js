...
angular
  ...
  .config([
    ...
    function ($routeProvider) {
      $routeProvider
        .when('/login', { template: '<authentication-component />' })
        .when('/labels', { template: '<labels-component />' })
        .when('/filters', { template: '<filters-component />' })
        .when('/export', { template: '<export-component />' })
        .otherwise({
          redirectTo: '/labels'
        })
    }
  ...
  .service('gmailService', [
    ...
    '$timeout',
    '$interval',
    function ($rootScope, $http, $timeout, $interval) {
      this.authenticate = () => {
        var req = {
          method: 'GET',
          url: '/api/auth/gmail'
...
function run($rootScope, $location, $cookies, $http, authService) {
  ...
  $rootScope.$on('$locationChangeStart', (_event, _next, _current) => {
